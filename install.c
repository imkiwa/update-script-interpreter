...
Value* MountFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    printf("挂载 %s(%s) 到 %s(%s)\n", location, partition_type, mount_point, fs_type);
    /* end */
...
//     }
    ...
    /* add by kiva */
    result = location;
    long long size = atoll(fs_size);
    printf("格式化 %s(%s) 为 %s(", location, partition_type, fs_type);
    if (size == 0)
    {
        printf("所有 字节");
    }
...
Value* SetPermFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (recursive) {
        ...
        for (i = 4; i < argc; ++i) {
            ...
            /* add by kiva */
            printf("设置 %s 的权限为(uid=%d, gid=%d, dirmode=%d, filemode=%d)\n", args[i], uid, gid, dir_mode, file_mode);
            /* end */
        }
    } else {
        ...
        for (i = 3; i < argc; ++i) {
            ...
            /* add by kiva */
            printf("设置 %s 的权限为(uid=%d, gid=%d, mode=%d)\n", args[i], uid, gid, mode);
            /* end */
        }
    }
    result = strdup("");
